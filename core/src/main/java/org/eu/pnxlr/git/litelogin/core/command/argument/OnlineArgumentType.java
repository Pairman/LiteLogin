...
public class OnlineArgumentType implements ArgumentType<OnlineArgumentType.OnlineArgument> {
    ...
    public OnlineArgument parse(StringReader reader) throws CommandSyntaxException {
        ...
        if (uuid == null) {
            ...
            if (uuid == null) {
                ...
                throw UniversalCommandExceptionType.create(
                        org.eu.pnxlr.git.litelogin.api.internal.util.ValueUtil.transPapi("§cCould not find an online identity named §e{online_name}§c for service §e{service_name}§8(sid = §e{service_id}§8)§",
        ...
        if (there == null) {
            reader.setCursor(i);
            throw UniversalCommandExceptionType.create(
                    org.eu.pnxlr.git.litelogin.api.internal.util.ValueUtil.transPapi("§cCould not find an online identity with UUID §e{online_uuid}§c for service §e{service_name}§8(sid = §e{service_id}§8)§",
                            new Pair<>("service_name", serviceConfig.getName()),
                            new Pair<>("service_id", serviceConfig.getId()),
