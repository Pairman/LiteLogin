package org.eu.pnxlr.git.litelogin.velocity.injector.handler;
 ...
 */
@Getter()
public class InitialLoginSessionHandlerBridge {
    ...
    private final CoreAPI coreApi; // 这个不是
    ...
    public InitialLoginSessionHandlerBridge(InitialLoginSessionHandler initialLoginSessionHandler, CoreAPI coreApi) {
    ...
    public void handle(EncryptionResponsePacket packet) throws Throwable {
        ...
        if (login == null) {
            throw new IllegalStateException("No ServerLogin packet received ");
        }
        if (this.verify.length == 0) {
            throw new IllegalStateException("No EncryptionRequest packet sent ");
        }
        ...
        try {
            ...
            if (this.inbound.getIdentifiedKey() != null) {
                ...
                if (!playerKey.verifyDataSignature(packet.getVerifyToken(), this.verify, Longs.toByteArray(packet.getSalt()))) {
                    throw new IllegalStateException("Invalid client public ");
                }
            } else {
                ...
                if (!MessageDigest.isEqual(this.verify, decryptedSharedSecret)) {
                    throw new IllegalStateException("Unable to successfully decrypt the verification ");
                }
            }
            ...
            coreApi.getPlugin().getRunServer().getScheduler().runTaskAsync(() -> {
                LoginAuthResult result = (LoginAuthResult) coreApi.getAuthHandler().auth(username, serverId, playerIp);
                try {
                    if (mcConnection.getChannel().eventLoop().submit(() -> {
                        ...
                        if (result.getResult() == AuthResult.Result.ALLOW) {
                            ...
                            try {
                                SkinRestorerResult restorerResult = coreApi.getSkinRestorerHandler().doRestorer(result);
                                if (restorerResult.getThrowable() != null) {
                                    LoggerProvider.getLogger().error("An exception occurred while processing skin restoration.", restorerResult.getThrowable());
                                }
                                ...
                                LoggerProvider.getLogger().debug("An exception occurred while processing skin restoration.", e);
                            }
