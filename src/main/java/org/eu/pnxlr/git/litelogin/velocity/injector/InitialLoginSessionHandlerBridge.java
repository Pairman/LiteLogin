...
import org.eu.pnxlr.git.litelogin.api.internal.result.AuthResult;
...
public class InitialLoginSessionHandlerBridge {
    ...
    public InitialLoginSessionHandlerBridge(InitialLoginSessionHandler initialLoginSessionHandler, Core core) {
    ...
    public void handle(EncryptionResponsePacket packet) throws Throwable {
        ...
        try {
            ...
            core.getScheduler().runTaskAsync(() -> {
                LoginAuthResult result = core.getAuthHandler().auth(username, serverId, playerIp);
                try {
                    if (mcConnection.getChannel().eventLoop().submit(() -> {
                        ...
                        if (result.getResult() == AuthResult.Result.ALLOW) {
                            ...
                            try {
                                SkinRestorerResult restorerResult = core.getSkinRestorerHandler().doRestorer(result);
